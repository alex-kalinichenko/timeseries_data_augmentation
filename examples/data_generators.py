...
def generate_test_data_1() -> (pd.DataFrame, datetime):
    """
    returns pd.Dataset of stationary  process with few "peaks" in train and test parts
    like this:
    time                    x             y
    ...
    also returns moment of time to split train and test dataset
    """
...
def generate_test_data_2() -> (pd.DataFrame, datetime):
    """
    returns tuple pd.Dataset - generated data and datetime where you should split dataset
...
def gauss_with_exp_acf_gen(
    *,
    ...
    Returns
    ----------
    xi : array of elements of a pseudo-random process with a given correlation function
    """
    np.random.seed(seed)
...
def generate_test_data_5() -> (pd.DataFrame, datetime):
    ...
    returns tuple of:
     ...
     - datetime where you should split dataset into train and test parts
    """
...
def get_data_from_file(filename):
    """
    read data from file, returns tuple:
     - pd.Dataset - generated data
     - datetime where you should split dataset into train and test parts
    """
    dates_dict = {
        "6792": datetime(2021, 9, 20),
        "8022": datetime(2021, 8, 15),
        "8212": datetime(2021, 9, 1),
        "678": datetime(2022, 1, 1),
        "679": datetime(2021, 11, 1),
        "680": datetime(2021, 8, 1),
        "710": datetime(2022, 1, 15),
        "801": datetime(2021, 7, 1),
        "802": datetime(2021, 8, 1),
        "818": datetime(2021, 11, 10),
        "819": datetime(2022, 2, 2),
        "905": datetime(2022, 2, 1),
        "long0945": datetime(2018, 1, 1),
    }
